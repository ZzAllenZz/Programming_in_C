...
void Instruct(Program *p)
{
    ...
    if(strsame(p->array[p->cw],"PRINT") || \
    strsame(p->array[p->cw],"PRINTN")){
        parse_print(p);
    ...
    if(strsame(p->array[p->cw],"IFEQUAL") || \
    strsame(p->array[p->cw],"IFGREATER")){
        parse_ifcondition(p);
...
void interp_innum(Program *p)
{
    ...
    for(i=ZERO;i<len;i++){
        if((str[i]<'0'||str[i]>'9')&&str[i]!='.'){
            free(str);
            ERROR_2("Expect only digit or . in NUM",\
            p->array[p->cw],p->cw);
        }
        if(str[i] =='.'){
            flag++;
        ...
        if(flag>1){
            free(str);
            ERROR_2("No more than 1 dot in NUM",\
            p->array[p->cw],p->cw);
        }
    }
    insert_map(p,str);
